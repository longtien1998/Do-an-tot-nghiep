...
<div class="container-fluid">
    ...
    <div class="form-group row justify-content-between m-0 p-0">
        <div class="form-group col-12 my-4">
            <h5>Bộ Lọc</h5>
            <form action="{{ route('albums.list') }}" class="row align-middle" method="GET" id="itemsPerPageForm">
                @csrf
                ...
                <div class="col-6 col-sm-2">
                    <label for="">Theo Ca sĩ</label>
                    <select name="filterSinger" class="form-select" onchange="submitForm()">
                        <option value="">Chọn Ca sĩ</option>
                        @foreach ($singers as $singer)
                        <option value="{{ $singer->id }}"
                            {{ request()->input('filterSinger') == $singer->id ? 'selected' : '' }}>
                            {{ $singer->singer_name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="col-6 col-sm-2">
                    ...
                    <input type="date" name="filterCreate" class="form-control"
                        value="{{ request()->input('filterCreate') ? request()->input('filterCreate') : '' }}"
                        onchange="submitForm()">
                </div>
            </form>
        </div>
        ...
        <div class="col-sm-6 my-3">
            <form action="{{route('albums.delete-list')}}" class="d-inline float-end" method="post" id="form-delete">
                ...
                <button type="submit" class="btn btn-danger" onclick="return confirm('Xác nhận xóa album đã chọn?')">Xóa Album</button>
            </form>
        </div>
    </div>
