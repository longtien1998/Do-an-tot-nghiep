            ...
            <div class="d-flex align-items-center justify-content-end">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{route('publishers.index')}}" class="text-decoration-none">Nhà xuất bản</a>
                        </li>
...
<div class="container mt-5">
    <h1 class="text-center mb-4">Quản lý File logo</h1>
    <div class="form-group row justify-content-between m-0 p-0">
        <div class="col-sm-3 my-3">
            <div>Đã chọn <strong id="total-songs">0</strong> mục</div>
        </div>
        <div class="col-sm-6 text-center my-3">
            <form action="{{route('publishers.destroy-list-logo')}}" class="d-inline float-end" method="post" id="form-delete">
                @csrf
                <input type="text" value="" name="delete_list" id="songs-delete" class="delete_list" hidden>
                <button type="submit" class="btn btn-danger" onclick="return confirm('Xác nhận xóa logo đã chọn?')">Xóa hình ảnh logo</button>
            </form>
        </div>
