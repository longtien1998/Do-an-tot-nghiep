...
<div class="page-breadcrumb">
    <div class="row">
        <div class="col-5 align-self-center">
            <h4 class="page-title">Danh sách Quyền hạn</h4>
        </div>
            ...
            <div class="d-flex align-items-center justify-content-end">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Danh sách Quyền hạn</li>
                    </ol>
                </nav>
            </div>
        </div>
...
<div class="container-fluid">
    <div class="form-group row justify-content-between m-0 p-0">
        <div class="col-sm-6 my-3">
            <a href="{{route('permissions.index')}}" class="btn btn-outline-success"> Tất cả Quyền hạn</a>
            <a href="{{route('permissions.create')}}" class="btn btn-success">Thêm Quyền hạn</a>
        </div>
        <div class="col-sm-6 my-3">
            <form class="search-form float-end" action="{{route('permissions.search')}}" method="post">
                @csrf
                <input type="text" name="search" placeholder="Tên, tên khác, module ..." required />
    ...
    <div class="form-group row justify-content-between align-content-center m-0 p-0">
        <div class="form-group col-12 my-auto">
            <h5>Bộ Lọc</h5>
            <form action="{{route('permissions.index')}}" class="row align-middle" method="post" id="itemsPerPageForm">
                @csrf
        ...
        <div class="col-sm-6 my-auto">
            <form action="{{route('permissions.delete-list')}}" class="d-inline float-end" method="post" id="form-delete">
                @csrf
                <input type="text" value="" name="delete_list" id="songs-delete" class="delete_list" hidden>
                <button type="submit" class="btn btn-danger" onclick="return confirm('Xác nhận xóa Quyền hạn đã chọn?')">Xóa Quyền hạn</button>
            </form>
        </div>
        ...
        <table class="table text-center table-hover" id="myTable">
            <thead>
                <tr>
                    ...
                    <th scope="col" onclick="sortTable(3)">Tên Quyền hạn <span class="sort-icon"> ⬍ </span></th>
                    <th scope="col" onclick="sortTable(4)">Bí danh <span class="sort-icon"> ⬍ </span></th>
                    <th scope="col" onclick="sortTable(4)">Module <span class="sort-icon"> ⬍ </span></th>
            ...
            <tbody>
                ...
                <tr>
                    <td><input type="checkbox" class="check_list" value="{{$permission->id}}"></td>

                    <th scope="row">{{$permissions->firstItem() + $index}}</th>
                    <td>{{$permission->id}} </td>
                    <td>{{$permission->alias}} </td>
                    <td>{{$permission->name}}</td>
                    <td>{{$permission->module->slug}}</td>
                    <td>{{$permission->created_at->format('d-m-Y')}}</td>
                    <td scope="row-3">
                        <a href="{{route('permissions.edit',$permission->id)}}" class="btn btn-link btn-outline-warning"> <i class="fa-solid fa-edit"></i></a>
                        <form action="{{route('permissions.delete',$permission->id)}}" method="post" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" data-bs-toggle="tooltip" title="" class="btn btn-link btn-outline-danger" data-original-title="Remove" onclick="return confirm('Xác nhận xóa Quyền hạn?')">
