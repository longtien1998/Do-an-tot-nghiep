    ...
    <div class="container-fluid">
        ...
        <div class="form-group row justify-content-between align-content-center m-0 p-0">
                ...
                <form action="{{ route('singer.index') }}" class="row align-middle" method="post" id="itemsPerPageForm">
                    ...
                    <div class="col-6 col-sm-2">
                        ...
                        <select name="indexPage" id="indexPage" class="form-select" onchange="submitForm()">
                            ...
                            <option value="500" {{ request()->input('indexPage') == 500 ? 'selected' : '' }}>500
                            </option>
                            <option value="1000" {{ request()->input('indexPage') == 1000 ? 'selected' : '' }}>1000
                            </option>
                        </select>
                    </div>
                    <div class="col-6 col-sm-2">
                        ...
                        <select name="filterGenDer" id="filterGenDer" class="form-select" onchange="submitForm()">
                            <option value="{{ request()->input('filterGenDer') }}">
                                {{ request()->input('filterGenDer') ?? 'Chọn giới tính' }}</option>
                    ...
                    <div class="col-6 col-sm-2">
                        ...
                        <input type="date" name="filterCreateStart" class="form-control"
                            value="{{ request()->input('filterCreateStart') ? request()->input('filterCreateStart') : '' }}"
                            onchange="submitForm()">
                    </div>
                    <div class="col-6 col-sm-2">
                        <label for="">Đến: Ngày tạo</label>
                        <input type="date" name="filterCreateEnd" class="form-control"
                            value="{{ request()->input('filterCreateEnd') ? request()->input('filterCreateEnd') : '' }}"
                            onchange="submitForm()">
                    </div>
                </form>
            </div>
            ...
            <div class="col-sm-6 my-auto">
                <form action="{{ route('singer.delete-list') }}" class="d-inline float-end" method="post"
                    id="form-delete">
                    @csrf
                    <input type="text" value="" name="delete_list" id="songs-delete" class="delete_list" hidden>
                    <button type="submit" class="btn btn-danger"
                        onclick="return confirm('Xác nhận xóa Ca sĩ đã chọn?')">Xóa Ca
                        sĩ</button>
                </form>
            </div>
                    ...
                    @if (count($singers))
                        @foreach ($singers as $index => $singer)
                            <tr>
                                ...
                                <td><img width="50px" height="50px" src="{{ $singer->singer_image }}"
                                        alt="">
                                </td>
                                <td><img width="100px" height="50px" src="{{ $singer->singer_background }}"
                                        alt=""></td>
                                ...
                                <td scope="row-3">
                                    <a href="{{ route('singer.edit', $singer->id) }}"
                                        class="btn btn-link btn-outline-success"> <i class="fa-solid fa-edit"></i></a>
                                    <form action="{{ route('singer.delete', $singer->id) }}" method="post"
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" data-bs-toggle="tooltip" title=""
                                            class="btn btn-link btn-outline-danger" data-original-title="Remove"
                                            onclick="return confirm('Xác nhận xóa Ca sĩ?')">
