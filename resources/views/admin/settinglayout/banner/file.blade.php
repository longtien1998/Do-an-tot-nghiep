...
<div class="container mt-5">
    ...
    <div class="form-group row justify-content-between m-0 p-0">
        ...
        <div class="col-sm-6 text-center my-3">
            <form action="{{route('banner.destroy-list-banner')}}" class="d-inline float-end" method="post" id="form-delete">
                @csrf
                <input type="text" value="" name="delete_list" id="songs-delete" class="delete_list" hidden>
                <button type="submit" class="btn btn-danger" onclick="return confirm('Xác nhận xóa banner đã chọn?')">Xóa hình ảnh banner</button>
            </form>
        </div>
    ...
    <table class="table table-bordered table-striped">
        ...
        <tbody>
            ...
            <tr>
                <td><input type="checkbox" class="check_list{{$image['in_use'] ? '1' : ''}}" value="{{$image['path']}}" @if ($image['in_use']) disabled @endif></td>
                <td>{{ $stt++ }}</td>
                ...
                <td>
                    ...
                    @else
                    <form action="{{ route('banner.destroy_file') }}" method="POST" onsubmit="return confirm('Bạn có chắc muốn xóa ảnh này?');">
                        @csrf
